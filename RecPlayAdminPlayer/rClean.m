	...
	if (tempName)
	  {
		...
		[CleanZeilenDic setObject:[NSNumber numberWithInt:NamenWeg] forKey:@"namenweg"];//Option 'Lšschen' vormerken
	  }
...
- (IBAction)reportExport:(id)sender
{
		...
		if ([tempTitelArray count])
		{
         ...
         /*
			...
			*/
			...
			[CleanOptionDic setObject:[NSNumber numberWithLong:[[ExportVariante selectedCell]tag]] forKey:@"exportvariante"];
			[CleanOptionDic setObject:[NSNumber numberWithLong:[[ExportFormatVariante selectedCell]tag]] forKey:@"exportformatvariante"];
			[CleanOptionDic setObject:[NSNumber numberWithLong:[[ExportAnzahlPop selectedItem]tag]] forKey:@"exportanzahl"];
	...
	else
	{
		...
		[Warnung addButtonWithTitle:@"OK"];
		//[Warnung addButtonWithTitle:@"Cancel"];
		[Warnung setMessageText:@"Fehler beim Exportieren:"];
		[Warnung setInformativeText:FehlerString];
		[Warnung setAlertStyle:NSWarningAlertStyle];
		[Warnung runModal];
				  ...
				  if (nurTitelZuNamenOption)
				  ...
				  else
					{
					  ...
					  if ([NamenIndexSet containsIndex:row])
						{
						  //[NamenIndexSet removeIndex:row];//Name schon im Set: lšschen
						  NamenWeg=1;//Lšschen anzeigen
							  //[[NamenArray objectAtIndex:row]setObject:[NSNumber numberWithInt:0] forKey:auswahl];
						}
					  else
