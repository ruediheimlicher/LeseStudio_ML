...
- (BOOL)LeseboxValidAnPfad:(NSString*)derLeseboxPfad aufSystemVolume:(BOOL)istSystemVolume
{
  ...
  else
  {
     ...
     [Warnung addButtonWithTitle:@"Anlegen"]; // 1000
     [Warnung addButtonWithTitle:@"Beenden"]; // 1001
...
- (BOOL)ProjektOrdnerEinrichtenAnPfad:(NSString *)derProjektPfad
{
   ...
   BOOL istOrdner;
   if (!( [Filemanager fileExistsAtPath:derProjektPfad isDirectory: &istOrdner] && istOrdner))
   {
      
      OrdnereinrichtenOK=[Filemanager createDirectoryAtPath:derProjektPfad  withIntermediateDirectories:NO attributes:NULL error:NULL];
      if (!OrdnereinrichtenOK)//Kein Ordner für das Projekt
      {
         NSAlert *Warnung = [[NSAlert alloc] init];
         [Warnung addButtonWithTitle:@"Ueberspringen"];
         ...
         NSString* InfoString2=@"\rDer Name ist eventuell schon vorhanden.";
	...
	if (OrdnereinrichtenOK)//alles OK
	{
		...
		[Warnung addButtonWithTitle:@"Aus NamenListe"];
		[Warnung addButtonWithTitle:@"Einzelne Namen eingeben"];
		...
		else
		{
			...
			NSString* s2=@" hat keinen gueltigen Namen.";
...
- (void)UNamenEinsetzenAktion:(NSNotification*)note
{
   ...
   //NSLog(@"*UNamenEinsetzenAktion  eingesetzteNamenArray:%@",[eingesetzteNamenArray description]);
	...
	if ([fehlendeOrdnerArray count])
	{
		NSAlert *Warnung = [[NSAlert alloc] init];
		[Warnung addButtonWithTitle:@"OK"];
		NSString* s1=@"Der Ordner für Projekt %@ konnte nicht angelegt werden.";
...
//	NSModalSession TimeoutSession=[NSApp beginModalSessionForWindow:[UTimeoutDialogPanel window]];
	...
	NSString* string1=@"Keine Altivität seit einiger Zeit";
	NSString* string2=[NSString stringWithFormat:@"Logout in %d secconds",Zeit];
